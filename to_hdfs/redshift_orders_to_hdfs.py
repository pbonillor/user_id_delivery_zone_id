...
def funcion_orders(app_args):
    try:
        ...
        dataframe_postgresql_orders_details_options = sqlContext.read.format("jdbc").option("url", "jdbc:postgresql://redshift-bi.peya.co:5439/peyabi").option("dbtable", "temporary.orders_to_hdfs").option("user", "bigdata_etl").option("password", "zej4uoI9YR2m0Nxh84wi").option("useSSL", "false").load()
		# con el dataframe_postgresql_dim area se crea la tabla temporal de spark orders_details_options_to_hdfs
        dataframe_postgresql_orders_details_options.createOrReplaceTempView("orders")
        ...
        for country in to_list:
            ...
            for date in date_generated:
                ...
                sqlDF_by_country = sqlContext.sql("SELECT * FROM orders WHERE orders.country_id = " + str(country[0]) + " and orders.hdfs_date = '" + date.strftime("%Y-%m-%d") + "'")
                # se elimina el contenido anterior en hdfs de orders_details_options por pais
                os.system('hdfs dfs -rm -f -R /'+country[2]+'/orders/'+date.strftime("%Y-%m-%d")+'/')
                ...
                print 'Inserted Data /'+country[2]+'/orders/'+date.strftime("%Y-%m-%d")+'/'
